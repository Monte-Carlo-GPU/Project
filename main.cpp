...
int main() {
    ...
    cout << "Time to maturity in years (for instance 1): ";
    if (!safeInput(T) || T <= 0.0f) {
        cerr << "Error: T must be > 0." << endl;
        return EXIT_FAILURE;
    }

    cout << "Annual drift (for instance 0.01): "; 
    if(!safeInput(mu)) {
    ...
    size_t trajectories_done = 0;

    // To save the trajectories, we will take several from the first batch later, if possible.
    size_t N_TRAJ_TO_SAVE = std::min((size_t)1000, N_PATHS);
    vector<float> first_batch_normals;
    bool first_batch_copied = false;

    for (size_t batch_idx = 0; batch_idx < total_batches; batch_idx++) {
        size_t current_batch_size = std::min(batch_size, N_PATHS - trajectories_done);
        size_t current_N_NORMALS = current_batch_size * N_STEPS;

        // Generate normal numbers for the current batch
        CURAND_CHECK(curandGenerateNormal(curandGenerator, d_normals_batch, current_N_NORMALS, 0.0f, sqrt(T/N_STEPS)));
        ...
        vector<float> h_S(current_batch_size);
        ...
        // Let's save the data for the trajectories from the first batch, if necessary.
        if (!first_batch_copied && N_TRAJ_TO_SAVE > 0 && current_batch_size >= N_TRAJ_TO_SAVE) {
            first_batch_normals.resize(N_TRAJ_TO_SAVE * N_STEPS);
            CUDA_CHECK(cudaMemcpy(first_batch_normals.data(), d_normals_batch,
                                  N_TRAJ_TO_SAVE * N_STEPS * sizeof(float),
                                  cudaMemcpyDeviceToHost));
            first_batch_copied = true;
        }
    ...
    // Saving multiple trajectories to trajectories.csv (from the first batch if available)
    if (first_batch_copied && N_TRAJ_TO_SAVE > 0) {
        ...
        for (size_t i = 0; i < N_TRAJ_TO_SAVE; i++) {
            ...
            for (size_t step = 1; step <= N_STEPS; step++) {
                double Z = first_batch_normals[i*N_STEPS + (step-1)];
