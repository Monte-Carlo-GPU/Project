...
#ifndef M_SQRT1_2
#define M_SQRT1_2 0.70710678118654752440  // Значение 1/sqrt(2)
#endif
...
int main() {
    ...
    // Генерация нормальных чисел и запуск ядра на GPU для всех траекторий
    // Будем считать, что все N_PATHS <= batch_size для упрощения (или вы уже выбрали такие параметры)
    // Если N_PATHS > batch_size, будет идти пакетная обработка, но предполагается один пакет для простоты
    ...
    // Аналитическая цена по Блэку–Шоулзу
    ...
    // Сохранение нескольких траекторий в trajectories.csv
    size_t N_TRAJ_TO_SAVE = ( N_PATHS > 100 ) ? N_PATHS : 100;
    if (N_PATHS >= N_TRAJ_TO_SAVE) {
        // Скопируем нормальные числа для траекторий
